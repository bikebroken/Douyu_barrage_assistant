...
{
#ifdef _MSC_VER
	dy_keep_alive_req keep1_req(GetTickCount());
#else
	dy_keep_alive_req keep1_req(time(NULL));
#endif

	string data = keep1_req.transform_to_dy_string();
...
void dy_barrage_client::on_barrage(string data)
{
	...
	//将获取的UTF-8字符转化为Unicode
	const char* userName_utf8 = barr_res.sender_name.c_str();
...
string UTF82Unicode(const char *szU8)
{
	...
	string resString = ws2s(unicodeString);
